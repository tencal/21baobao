...
begin
	...
	Begin
		...
		delete from BO_Organizations
		delete from BO_Positions 	
		...
		delete from Gen_PromotionPrice
		delete from Gen_promotionCondition
		delete from Gen_Promotion
		
		delete from Gen_RoomItem
		delete from Gen_MarketCountry  --New Add
		
		delete from Gen_FreeEnquires
		...
		delete from Gen_Suppliers
		
		alter table Gen_RoomItem add EffectivDate date not null
		alter table Gen_RoomItemHistory add EffectivDate date not null
		alter table Gen_RoomItem add ExpiryDate date not null
		alter table Gen_RoomItemHistory add ExpiryDate date not null
		...
		ALTER TABLE Gen_RoomItem ADD CONSTRAINT [PK_Gen_RoomItem] PRIMARY KEY
			([RoomID],[ItemID],[MarketID],[PriceDate],[EffectivDate],[ExpiryDate])
    ...
    --inner join BO_Groups g on u.OrgCode=g.L_P_Code and g.IsCatalog=1
    ...
    insert into BO_UserInGroups(UserID,GroupID)
    select u.UserID,g.GroupID from [BO_Users] u
    inner join BO_Groups g on u.PositionCode=g.L_P_Code and g.IsCatalog=0 --职位组
    inner join BO_Groups gg on g.ParentID = gg.GroupID       --职位的上司或机构
    inner join BO_Groups ggg on gg.ParentID = ggg.GroupID	 --职位的机构
    inner join BO_Groups gggg on ggg.ParentID = gggg.GroupID	 --职位的机构
    inner join BO_Groups ggggg on gggg.ParentID = ggggg.GroupID	 --职位的机构
	...
	--插入单价
		...
		select MAX(id) as id, remark1,remark1,remark1  from [UAT].dbo.hif_fare 
